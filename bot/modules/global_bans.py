...
import bot.modules.sql.global_bans_sql as sql
from bot import dispatcher, OWNER_ID, SUDO_USERS, DEV_USERS, STRICT_GBAN, GBAN_LOGS, SUPPORT_CHAT, BAN_STICKER, UNBAN_STICKER
from bot.modules.helper_funcs.chat_status import user_admin, is_user_admin, sudo_plus
from bot.modules.helper_funcs.extraction import extract_user, extract_user_and_text
from bot.modules.helper_funcs.misc import send_to_list
from bot.modules.sql.users_sql import get_all_chats
...
def gban(bot: Bot, update: Update, args: List[str]):
    ...
    if int(user_id) in DEV_USERS:
        message.reply_text("That user is part of the DevUsers\nI can't act against our own.")
        return
    ...
    if user_id == bot.id:
        message.reply_text("You uhh...want me to ban myself?")
        return
    ...
    if GBAN_LOGS:
        ...
        send_to_list(bot, SUDO_USERS, f"Gban complete! (User banned in <code>{gbanned_chats}</code> chats)", parse_mode=ParseMode.HTML)
    ...
    try:
        bot.send_message(user_id,
                         ...
                         f" If you think that this was a mistake, you may appeal and check your ban @{SUPPORT_CHAT}. ",
                         parse_mode=ParseMode.HTML)
    except:
    ...
    if sql.is_user_gbanned(user_id):
        ...
        if should_message:
            update.effective_message.reply_text("<b>Alert:</b> This user is globally banned.\n"
                                                ...
                                                f"Appeal Owner: {SUPPORT_CHAT}", parse_mode=ParseMode.HTML)
