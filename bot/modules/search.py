# Image Reverse Search Module from @TheRealPhoenix all credits goes to him.
# Thanks @Zero_cool7870 helping me creating google search module.
# Used Spechide's API for google search.
...
@run_async
def find(bot: Bot, update: Update):
    args = update.message.text.strip().split(" ", 1)
    if len(args) == 1:
        update.message.reply_text("gib search query")
        return
    query = args[1]
    input_url = "https://bots.shrimadhavuk.me/search/?q={}".format(query)
    ...
    for result in response["results"]:
        ...
        url = result["url"]
        output_str += " => <a href='{}'>{}</a> \n\n".format(url,text)
    update.message.reply_text(output_str,parse_mode="HTML")

__help__ = """
- /reverse: Does a reverse image search of the media which it was replied to.
- /search <input>: To google search the INPUT given.
- /find <input>: Same as /search
"""

__mod_name__ = "Search"

REVERSE_HANDLER = DisableAbleCommandHandler("reverse", reverse, pass_args=True, admin_ok=True)
FIND_HANDLER = DisableAbleCommandHandler(["find", "search"], find)
