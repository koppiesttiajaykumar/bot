...
from bot import dispatcher, OWNER_ID, SUDO_USERS, GBAN_LOGS, LOGGER
from bot.modules.helper_funcs.handlers import CMD_STARTERS
from bot.modules.helper_funcs.misc import is_module_loaded, send_to_list
from bot.modules.helper_funcs.chat_status import is_user_admin
from bot.modules.helper_funcs.extraction import extract_user, extract_unt_fedban, extract_user_fban
...
def new_fed(bot: Bot, update: Update):
	...
	if not fednam == '':
		...
		if not x:
			update.effective_message.reply_text("Can't federate! Please contact @DAvinash97 if the problem persist.")
			return
...
def join_fed(bot: Bot, update: Update, args: List[str]):
	...
	if len(args) >= 1:
		...
		if not x:
			message.reply_text("Failed to join federation! Please contact @DAvinash97 should this problem persist!")
			return
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
	...
	if fban:
		...
		if not x:
			message.reply_text("Failed to ban from the federation! If this problem continues, contact @DAvinash97.")
			return
...
def set_frules(bot: Bot, update: Update, args: List[str]):
	...
	if len(args) >= 1:
		...
		if not x:
			update.effective_message.reply_text("Whoa! There was an error while setting federation rules! If you wondered why please ask it in @DAvinash97 !")
			return
		...
		for chat in chat_list:
			...
			try:
				...
				try:
					...
					LOGGER.info("Chat {} has left fed {} because I was kicked".format(chat, fedinfo['fname']))
					continue
				failed += 1
